local Helper = require("ponokai-pro.color_helper")
local Util = require("ponokai-pro.util")
local Config = require("ponokai-pro.config")
local Colorscheme = require("ponokai-pro.colorscheme")

---@class PonokaiProTheme
---@field mini ponokai-pro.theme.plugins.mini
local M = {}

setmetatable(M, {
  __index = function(_, k)
    local plugin = require("ponokai-pro.theme.plugins" .. k)
    return plugin
  end,
})
...
local function get_hl_group_tbl(colorscheme)
  local editor = require("ponokai-pro.theme.editor").setup(colorscheme, Config, Helper)
  local syntax = require("ponokai-pro.theme.syntax").setup(colorscheme, Config, Helper)
  local semantic_tokens = require("ponokai-pro.theme.semantic_tokens").setup(colorscheme, Config, Helper)
  local extra = require("ponokai-pro.theme.extra").setup(colorscheme, Config, Helper)
  ...
  for _, name in ipairs(PLUGINS) do
    local config_ok, plugin = xpcall(require, function(...)
      ...
      return ...
    end, "ponokai-pro.theme.plugins." .. name)
    if config_ok then
...
---@return HighlightGroupTbl
M.setup = function()
  local devicons = require("ponokai-pro.devicons")
