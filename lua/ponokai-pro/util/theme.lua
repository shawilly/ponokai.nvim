...
---@class ponokai-pro.util.theme
local M = {}

local function load_autocmds()
  local ok, _ = pcall(require, "ponokai-pro.autocmds")
  if not ok then
    Util.log("Failed to load ponokai-pro.autocmds", "error")
  end
end
...
local function get_real_color(hex_color, base)
  ...
  local filter = require("ponokai-pro.colorscheme").filter
  ---@module "ponokai-pro.colorscheme.palette.pro"
  local c = require("ponokai-pro.colorscheme.palette." .. filter)
  if base == nil then
    base = c.background
  end
...
function M.load(hl_group_tbl)
  ...
  vim.g.colors_name = "ponokai-pro"

  M.draw(hl_group_tbl)

  local bufferline_icon_hl_group_tbl = require("ponokai-pro.theme.plugins.bufferline").setup_bufferline_icon()
